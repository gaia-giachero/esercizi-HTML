...
* [Form Data Collection](#form-data-collection)
...
* **Comprehensive form**: integration with SheetDB API (Google Sheets);
...
└── 03-html-form/                            # unit3 folder
        ├── astronaut-application-data.xlsx  # google sheet form to save data
...
| Input Ttpe| Purpose | Example Fileds|
|-----------|---------|---------------|
...
|`tel`| Telephone number | Phone number (maxlenght 10) |
...
* **Form validation**: required fields, min/max values, and specific input types;
* **External CSS**: linked stylesheet for custom styling.
...
2. **Double-click** on `index.html`
3. The page will open in your default browser
4. You'll see the complete astronaut application form
...
## 🔍 What It Demonstrates
This project showcases a **comprehensive web form for astronaut applications** using fundamental HTML form elements:

### 1. Heading and Structure
* **h1**: Main page title ("Astronaut Application")
* **p**: Introductory text prompting users to complete the form
...
| `text` | Single-line text entry | Name, Surname, Nationality, Address |
| `number` | Numeric values with constraints | Age (min 18), Body weight (max 100kg) |
| `radio` | Single selection from options | Gender selection |
| `color` | Color picker | Hair color |
| `email` | Email validation | Email address |
...
* **Min/max values**: Age ≥18, Body weight ≤100kg
* **Input patterns**: Email format validation
* **Character limits**: Telephone (10 chars), Biography (255 chars)
* **File type restrictions**: Only PDF files accepted for curriculum
* **Default selections**: "Man" pre-selected for gender, placeholder for eye color dropdown
...
|`title` | "Astronaut Application" - shows in browser tab |
|`description` | "Apply now for the upcoming space mission. Fill out your academic and physical profile to join our astronaut program." |
|`og:type` | Defines content as "website" for social media |
|`og:title` | "Astronaut Application" - title shown when shared |
|`og:description` | "Have you ever dreamed of the stars? Submit your application for the next space mission today." |
|`og:image` | Custom astronaut image displayed when shared on social media |
...
| `<fomr>` | Form container | Contains all input fields and submit |
...
| `<input>` | Various input types | tezt, number, radio, color, email, tel, checkbox, file |
| `<select>` | Dropdown menu | Eye color selection |
...
| `<b>` | Bold text | Filedset legend labels |

## Key Attributes Used:
| Attribute | Purpose | Example |  
|-----------|---------|---------|
| `action` | Form submission URL | SheetBD API endpoint and Wbhook option |
| `method` | HTTP method for submission | "POST" |
| `target` | Where to opern response | "_blank" (new tab) |
...
| `type` | Input field type | tezt, number, email, radio, checkbox, etc. |
...
| `maxlenght` | Character limit | Telephone (10), Biography (255) |
...
| `accept` | File type restriction | ".pdf" for curriculm upload |
| `rows`/`cols` | Textarea dimensions | 6 rows x 100 columns for biography |
| `enctype` | Form econding type | "multipart/form-data" |
